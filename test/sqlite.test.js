...
test('Krusch Memory MCP - SQLite Integration', async (t) => {
  ...
  try {
    ...
    const addResult = await client.callTool({
      name: 'add_memory',
      arguments: { category: 'lessons', content: 'Krusch is a local memory system.' }
    });
    ...
    assert.strictEqual(searchResult.content[0].text.includes('Krusch is a local memory system.'), true);
